    ...
    <div style={{ background: C.black, color: C.text, fontFamily: 'Inter, system-ui, sans-serif', padding: 'var(--space-lg)', width: '100%', minHeight: '100%' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 'var(--space-md)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
          {/* Send message */}
          <div style={{ background: C.card, border: `0.5px solid ${C.border}`, borderRadius: 'var(--radius-card)', padding: 20, display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => setActiveTab('broadcast')} style={{ flex: 1, height: 40, borderRadius: 'var(--radius-sm)', border: `0.5px solid ${activeTab === 'broadcast' ? C.gold : C.border}`, background: activeTab === 'broadcast' ? `${C.gold}12` : C.line, color: activeTab === 'broadcast' ? C.gold : C.dim, fontFamily: '"Space Grotesk", Inter, sans-serif', fontWeight: 700, fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                <Globe size={14} /> Broadcast
              </button>
              <button onClick={() => setActiveTab('direct')} style={{ flex: 1, height: 40, borderRadius: 'var(--radius-sm)', border: `0.5px solid ${activeTab === 'direct' ? C.gold : C.border}`, background: activeTab === 'direct' ? `${C.gold}12` : C.line, color: activeTab === 'direct' ? C.gold : C.dim, fontFamily: '"Space Grotesk", Inter, sans-serif', fontWeight: 700, fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                <Building2 size={14} /> Direct
              </button>
            </div>

            {activeTab === 'direct' && (
              <select value={selectedIsp} onChange={e => setSelectedIsp(e.target.value)} style={{ height: 44, padding: '0 14px', borderRadius: 12, border: `1px solid ${C.border}`, background: C.line, color: C.text, fontSize: 13, fontFamily: 'inherit', outline: 'none' }}>
            ...
            <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Title" style={{ height: 44, padding: '0 14px', borderRadius: 12, border: `1px solid ${C.border}`, background: C.line, color: C.text, fontSize: 13, fontFamily: 'Inter, system-ui, sans-serif', outline: 'none' }} />

            <textarea value={message} onChange={e => setMessage(e.target.value)} placeholder="Message" rows={4} style={{ padding: 14, borderRadius: 12, border: `1px solid ${C.border}`, background: C.line, color: C.text, fontSize: 13, fontFamily: 'Inter, system-ui, sans-serif', outline: 'none', resize: 'vertical' }} />
          ...
          {/* History */}
          <div style={{ background: C.card, border: `0.5px solid ${C.border}`, borderRadius: 'var(--radius-card)', padding: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            {loadingHistory && history.length === 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                {[1, 2, 3].map(r => (
                  <div key={r} style={{ padding: '14px 0', borderBottom: `1px solid ${C.line}` }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                      <div style={{ width: 12, height: 12, background: C.mute, borderRadius: '50%', animation: 'skel-pulse 2s ease-in-out infinite', animationDelay: `${r * 0.1}s` }} />
                      <div style={{ width: '40%', height: 13, background: C.mute, borderRadius: 4, animation: 'skel-pulse 2s ease-in-out infinite', animationDelay: `${r * 0.1}s` }} />
                    </div>
                    <div style={{ width: '80%', height: 10, background: C.mute, borderRadius: 4, marginTop: 6, animation: 'skel-pulse 2s ease-in-out infinite', animationDelay: `${r * 0.12}s` }} />
                    <div style={{ width: '50%', height: 10, background: C.mute, borderRadius: 4, marginTop: 4, animation: 'skel-pulse 2s ease-in-out infinite', animationDelay: `${r * 0.14}s` }} />
                  </div>
                ))}
              </div>
            ) : loadingHistory ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: C.dim, padding: 20 }}>
                <Loader2 size={14} style={{ animation: 'spin 1s linear infinite' }} /> Loading...
              </div>
            ) : history.length === 0 ? (
              <div style={{ padding: 30, textAlign: 'center', color: C.dim, fontSize: 13 }}>No messages sent yet.</div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 0, overflowY: 'auto', maxHeight: 520 }}>
                {history.map((h: any) => (
                  <div key={h.id} style={{ padding: '14px 0', borderBottom: `1px solid ${C.line}` }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          {h.type === 'broadcast' ? <Globe size={12} color={C.gold} /> : <Building2 size={12} color={C.dim} />}
                          <span style={{ fontSize: 13, fontWeight: 600, color: C.text }}>{h.title}</span>
                        </div>
                        <div style={{ marginTop: 6, fontSize: 12, color: C.dim, lineHeight: 1.5 }}>{h.message}</div>
                      </div>
                    </div>
                    <div style={{ marginTop: 8, display: 'flex', gap: 16, fontSize: 10, color: C.mute, fontFamily: '"DM Mono", monospace' }}>
