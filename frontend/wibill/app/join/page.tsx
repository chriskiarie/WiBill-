...
function JoinPageInner() {
  ...
  const handleSignup = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      resetToRegister(err?.message || 'An error occurred')
      return
    }

    // Pending approval → redirect immediately
  ...
  return (
    <div style={{
      ...
      {/* ─── REGISTER CARD (persists through exit animation) ─── */}
      {(phase === 'register' || (phase !== 'welcome' && phase !== 'loading')) && (
        <div style={{
          width: '100%', maxWidth: 420,
          transition: 'transform 0.65s cubic-bezier(0.76, 0, 0.24, 1), opacity 0.5s ease',
          transform: cardExiting ? 'translateX(-130%) rotate(-3deg)' : 'none',
            ...
            <form onSubmit={handleSignup} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <button
                ...
                style={{
                  ...
                  transform: buttonExiting ? 'translateX(140%) rotate(4deg)' : 'none',
      ...
      {/* ─── WELCOME ─── */}
      {phase === 'welcome' && (
        <div style={{ textAlign: 'center', animation: 'fade-up 0.7s ease-out forwards' }}>
          ...
          <div style={{
            marginTop: 32,
            animation: 'fade-up 0.7s ease-out 0.7s forwards',
            opacity: 0, transform: 'translateY(20px)',
          }}>
