...
export default function LoginPage() {
  ...
  // Check if invite token is in URL
  useEffect(() => {
  ...
  const handleSignup = async (e: FormEvent) => {
    ...
    try {
      ...
      let url = `${API}/api/auth/register`
      if (inviteToken) {
        url += `?token=${encodeURIComponent(inviteToken)}`
      }
      ...
      // If invite token, show success and redirect to login
      if (inviteToken) {
        setError('')
  ...
  const btn: React.CSSProperties = {
    width: '100%', padding: '14px', background: '#E8B84B', border: 'none',
    borderRadius: 9, color: '#000', fontFamily: 'Space Grotesk, sans-serif',
    ...
    <div style={{ minHeight: '100vh', background: '#030303', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ width: '100%', maxWidth: 420 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 36 }}>
          <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 34, fontWeight: 800, letterSpacing: '-1px', color: '#fff' }}>WiBill</div>
          <div style={{ fontSize: 13, color: '#666', marginTop: 2 }}>ISP Management Portal</div>
          {inviteToken && (
            <div style={{ marginTop: 12, padding: '8px 12px', background: '#0a2a0a', border: '0.5px solid #1a3a1a', borderRadius: 6, color: '#22c55e', fontSize: 11, fontWeight: 600 }}>
              ✓ Invite link active — Complete registration to launch your dashboard
            </div>
          )}
        </div>

        {/* Tabs - Hidden if invite token */}
        {!inviteToken && (
          <div style={{ display: 'flex', background: '#0a0a0a', border: '0.5px solid #161616', borderRadius: 10, padding: 4, marginBottom: 24, gap: 4 }}>
            {(['login', 'signup'] as const).map(t => (
              <button key={t} onClick={() => { setTab(t); setError('') }} style={{
                ...
                color: tab === t ? '#fff' : '#333',
                fontFamily: 'Space Grotesk, sans-serif', fontSize: 12, fontWeight: tab === t ? 700 : 400,
        ...
        <div style={{ background: '#080808', border: '0.5px solid #141414', borderRadius: 12, padding: 28 }}>
          {(!inviteToken && tab === 'login') ? (
            ...
            <form onSubmit={handleSignup} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div><label style={lbl}>ISP NAME</label><input style={inp} type="text" value={ispName} onChange={e => setIspName(e.target.value)} placeholder="e.g., Simply Networks" required /></div>
              <div><label style={lbl}>ISP SLUG (URL)</label><input style={inp} type="text" value={slug} onChange={e => setSlug(e.target.value)} placeholder="simply-networks" required /></div>
              <div><label style={lbl}>EMAIL</label><input style={inp} type="email" value={regEmail} onChange={e => setRegEmail(e.target.value)} placeholder="admin@simply.co.ke" required /></div>
              <div><label style={lbl}>PASSWORD</label><input style={inp} type="password" value={regPass} onChange={e => setRegPass(e.target.value)} placeholder="••••••••" required /></div>
              <div><label style={lbl}>PHONE (OPTIONAL)</label><input style={inp} type="tel" value={phone} onChange={e => setPhone(e.target.value)} placeholder="0712345678" /></div>
              {error && <div style={{ background: '#0d0404', border: '0.5px solid #2a0a0a', borderRadius: 7, padding: '10px 14px', color: '#f87171', fontSize: 12, fontFamily: 'DM Mono, monospace' }}>{error}</div>}
              <button type="submit" style={btn}>{loading ? 'Setting up...' : inviteToken ? 'LAUNCH MY DASHBOARD →' : 'CREATE ACCOUNT'}</button>
            </form>
          )}
        </div>
      </div>
