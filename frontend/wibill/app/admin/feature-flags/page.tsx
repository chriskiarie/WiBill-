...
const C = {
  ...
  gold: '#E8B84B', green: '#22c55e', red: '#ef4444', amber: '#f59e0b',
};
...
export default function FeatureFlagsPage() {
  ...
  function statusInfo(tenant: any) {
    if (!tenant.is_active) return { label: 'PENDING', color: C.amber };
      ...
      <div style={{ maxWidth: 1520, margin: '0 auto', padding: '0 28px 36px' }}>
        <header style={{ height: 68, display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `1px solid ${C.border}`, marginBottom: 24 }}>
          <div>
            ...
            <div style={{ marginTop: 4, fontSize: 12, color: C.muted }}>Premium management · Per-ISP feature control</div>
          </div>
        ...
        {loading && data.length === 0 ? (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: `160px 80px 80px repeat(${FEATURES.length}, 1fr)`, gap: 0 }}>
                ...
                {['Tier', 'Fee', ...FEATURES].map((f, i) => (
                  <div key={f} style={{ padding: '14px 10px', textAlign: 'center', borderBottom: `1px solid ${C.border}`, borderLeft: `1px solid ${C.borderSoft}` }}>
                    <div style={{ width: '60%', height: 10, margin: '0 auto', background: C.dim, borderRadius: 4, animation: 'skel-pulse 2s ease-in-out infinite', animationDelay: `${0.1 + i * 0.05}s` }} />
                  </div>
                ))}
                {[1, 2, 3, 4, 5].map(r => (
                  <React.Fragment key={r}>
                    <div style={{ padding: '12px 18px', display: 'flex', alignItems: 'center', gap: 8, borderBottom: `1px solid ${C.borderSoft}`, background: r % 2 === 0 ? C.panel2 : 'transparent' }}>
                    ...
                    {[1, 2, 3, 4, 5, 6, 7].map(i => (
                      <div key={i} style={{ padding: '8px 10px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderBottom: `1px solid ${C.borderSoft}`, borderLeft: `1px solid ${C.borderSoft}`, background: r % 2 === 0 ? C.panel2 : 'transparent' }}>
          ...
          <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 18, overflow: 'hidden' }}>
            <div style={{ display: 'grid', gridTemplateColumns: `160px 70px 70px 70px repeat(${FEATURES.length}, 1fr)`, gap: 0 }}>
              ...
              <div style={{ padding: '14px 8px', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: C.dim, textAlign: 'center', borderBottom: `1px solid ${C.border}`, borderLeft: `1px solid ${C.borderSoft}` }}>Status</div>
              <div style={{ padding: '14px 8px', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: C.dim, textAlign: 'center', borderBottom: `1px solid ${C.border}`, borderLeft: `1px solid ${C.borderSoft}` }}>Tier</div>
              <div style={{ padding: '14px 8px', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: C.dim, textAlign: 'center', borderBottom: `1px solid ${C.border}`, borderLeft: `1px solid ${C.borderSoft}` }}>Fee</div>
              {FEATURES.map(f => (
                <div key={f} style={{ padding: '14px 8px', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: C.dim, textAlign: 'center', borderBottom: `1px solid ${C.border}`, borderLeft: `1px solid ${C.borderSoft}` }}>
                  {FEATURE_LABELS[f]}
                </div>
              ))}
              {/* Rows */}
              {filtered.map((tenant, i) => {
                ...
                return (
                  <React.Fragment key={tenant.tenant_id}>
                    ...
                    <div style={{ padding: '8px 8px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderBottom: i < filtered.length - 1 ? `1px solid ${C.borderSoft}` : 'none', borderLeft: `1px solid ${C.borderSoft}`, background: i % 2 === 0 ? 'transparent' : C.panel2 }}>
                    ...
                    <div style={{ padding: '8px 8px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderBottom: i < filtered.length - 1 ? `1px solid ${C.borderSoft}` : 'none', borderLeft: `1px solid ${C.borderSoft}`, background: i % 2 === 0 ? 'transparent' : C.panel2 }}>
                      <button
                        ...
                        style={{
                          ...
                          background: isPremium ? 'rgba(232,184,75,0.15)' : C.panel2,
                    ...
                    <div style={{ padding: '8px 8px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderBottom: i < filtered.length - 1 ? `1px solid ${C.borderSoft}` : 'none', borderLeft: `1px solid ${C.borderSoft}`, background: i % 2 === 0 ? 'transparent' : C.panel2 }}>
                      <span style={{ fontFamily: '"DM Mono", monospace', fontSize: 12, color: isPremium ? C.gold : C.dim }}>{isPremium ? 'KES 0' : '—'}</span>
                    </div>
                    {FEATURES.map(f => {
                      ...
                      return (
                        <div key={key} style={{ padding: '8px 8px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderBottom: i < filtered.length - 1 ? `1px solid ${C.borderSoft}` : 'none', borderLeft: `1px solid ${C.borderSoft}`, background: i % 2 === 0 ? 'transparent' : C.panel2 }}>
                          {locked ? (
                            <span title="Upgrade to Premium to enable" style={{ color: '#3A3A37', display: 'flex', alignItems: 'center' }}>
                              <Lock size={14} />
                            </span>
                          ) : (
                            <button
                              ...
                              style={{
                                ...
                                background: enabled ? C.gold : '#2A2A27',
